...
The input video must be generated by Veo (validated by metadata).`,
  ...
  veo3 extend part2.mp4 --prompt "Part 3" --filename part3.mp4`,
	...
	_ = viper.BindPFlag("model", extendCmd.Flags().Lookup("model"))
	_ = viper.BindPFlag("output", extendCmd.Flags().Lookup("output"))
...
func runExtend(cmd *cobra.Command, args []string) error {
	...
	if !jsonFormat {
		fmt.Printf("➕ Extending video... (Operation: %s)\n", operation.ID)
	}
	...
	if operation.Status == veo3.StatusDone && !noDownload {
		if !jsonFormat {
			fmt.Println("✓ Video extension completed!")
		}
