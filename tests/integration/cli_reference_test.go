...
// TestGenerateCommand_ReferenceImageJSONOutput tests JSON output with reference images
func TestGenerateCommand_ReferenceImageJSONOutput(t *testing.T) {
	// Skip this test if no API key is available
	if os.Getenv("VEO3_API_KEY") == "" && os.Getenv("GEMINI_API_KEY") == "" {
		t.Skip("No API key available for integration test")
	}
	...
	rootCmd.SetArgs([]string{
		"generate",
		"--prompt", "Style-guided video generation",
		"--reference", "testdata/style1.jpg",
		"--reference", "testdata/style2.png",
		"--duration", "8",
		"--aspect-ratio", "16:9",
		"--json",
		"--no-download",
	...
	if result["success"].(bool) {
		...
		// Check reference image-specific metadata
		if metadata, ok := data["metadata"].(map[string]interface{}); ok {
			assert.Contains(t, metadata, "reference_image_paths")
			assert.Contains(t, metadata, "reference_image_count")

			// Verify reference image count
			refCount := metadata["reference_image_count"].(float64) // JSON numbers are float64
			assert.Equal(t, 2.0, refCount)
		}
	}
}
